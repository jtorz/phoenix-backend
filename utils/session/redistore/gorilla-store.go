...
func NewRediStore(size int, network, address, password string, keyPairs ...[]byte) (*RediStore, *redis.Pool, error) {
	pool := &redis.Pool{
		MaxIdle:     size,
	...
	store, err := NewRediStoreWithPool(pool, keyPairs...)
...
func (s *RediStore) New(r *http.Request, name string) (*sessions.Session, error) {
	...
	if c, errCookie := r.Cookie(name); errCookie == nil {
		c.Value, err = url.QueryUnescape(c.Value)
		if err != nil {
			return session, err
		}
...
func (s *RediStore) load(session *sessions.Session) (bool, error) {
	...
	{ // update session ttl
		...
		conn.Do("EXPIRE", s.keyPrefix+session.ID, age)
	}
