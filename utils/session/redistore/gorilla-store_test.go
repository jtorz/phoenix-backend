...
func TestRediStore(t *testing.T) {
	...
	// Round 1 ----------------------------------------------------------------
	{
		// RedisStore
		addr := setup()
		store, _, err := NewRediStore(10, "tcp", addr, "", []byte("secret-key"))
		if err != nil {
			t.Fatal(err.Error())
		}
		defer store.Close()
...
func TestPingGoodPort(t *testing.T) {
	store, _, _ := NewRediStore(10, "tcp", ":6379", "", []byte("secret-key"))
	defer store.Close()
	ok, err := store.ping()
	if err != nil {
		t.Error(err.Error())
...
func TestPingBadPort(t *testing.T) {
	store, _, _ := NewRediStore(10, "tcp", ":6378", "", []byte("secret-key"))
	defer store.Close()
	_, err := store.ping()
	if err == nil {
...
func ExampleRediStore() {
	// RedisStore
	store, _, err := NewRediStore(10, "tcp", ":6379", "", []byte("secret-key"))
	if err != nil {
		panic(err)
	}
	defer store.Close()
