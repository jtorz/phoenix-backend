...
{{- if $Col.IsPK}}:{{$Col.GoVarName}}/{{end}}{{end}}"remove_slash, http{{$.Entity.GoStruct}}.GetByID().Func())
	...
	apiGroup.PUT("/{{$.Entity.GoSlice | lowercase}}/{{$.Entity.GoStruct | lowercase}}/activate", http{{$.Entity.GoStruct}}.SetStatus(base.StatusActive).Func())
	apiGroup.PUT("/{{$.Entity.GoSlice | lowercase}}/{{$.Entity.GoStruct | lowercase}}/inactivate", http{{$.Entity.GoStruct}}.SetStatus(base.StatusInactive).Func())
